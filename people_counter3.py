...
class Person:
    ...
    def distance_from_last_x_positions(self, new_position, x):
        ...
        '''
    def trackline(self, new_position):
        (x1, y1) = self.positions[-1]
        (x2, y2) = new_position
        if (x2 - x1) > 250:
            return (x2 - x1)
...
def main():
    ...
    print ("Frames por segundo: %d", fps)
    ...
    while True:
        ...
        for (x, y, w, h) in peoples:
            ...
            cv2.imwrite("crops/roi"+str(id_img)+".png", crop)
            id_img+=1
            ...
            if lowest_closest_distance > 150:
                if closest_person_index is not None and people_list[closest_person_index].trackline(rectangle_center) is None:
                    closest_person_index = None
